...
<h3>스크립틀릿 내부에서객체를 별도로 생성하지 않아도 request/response에 접근할 수 있다</h3>
<h3>request와 response는 JSP의 내장객체에 해당한다(별도 생성이 필요 없음)</h3>

<table>
	<thead>
		<tr>
			<th colspan="3"> request주요 메서드</th>
		</tr>
		<tr>
			<td>형식</td>
			<td>기능</td>
		...
		<tr>
			...
			<td>request 저장된 내장객체를 이름으로 불러와서 Object로 반환한다</td>
			<td><%=request.getAttribute("today") %></td>
		</tr>
		<tr>
			<td>request.getCookies()</td>
			<td>요청에 담긴 쿠키를 배열 형태로 반환한다. null을 반환할수도 있다</td>
			<td><%=request.getCookies() %></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>
				요청받은 자원의 최상위 폴더 경로를 문자열로 반환한다<br>
