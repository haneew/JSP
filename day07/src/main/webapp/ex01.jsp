...
<h3>EL Tag에서는 null 여부를 체크하기 위해 empty 연산을 이용한다</h3>
	...
	<c:set var="s1" value="123"></c:set>
	<c:set var="s2" value="11"></c:set>
	<%=pageContext.getAttribute("s1").getClass().getSimpleName() %>
	
	<c:set var="s3" value="Hello "></c:set>		<%-- 만약 문자열을 더하고 싶다면 --%>
	<c:set var="s4" value="world "></c:set>
	<p>${s3 }${s4 }</p>
...
<!--  자바였다면 nullpointexp 이지만 오류가 생기지만 JSP는 생기지 않고 그냥 출력하는 경우가 생긴다. 
	객체는 찾았지만 메서드가 없는 경우 -->
