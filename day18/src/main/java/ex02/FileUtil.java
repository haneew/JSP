...
public class FileUtil {
	...
	// request를 전달받아서, 파일 및 데이터를 DTO에 담아서 반환하는 함수
	public Ex02DTO getDTO(HttpServletRequest request) throws IOException, ParseException {
		Ex02DTO dto = null;
		
		// 1) 저장할 디렉토리가 없으면 새로 생성해야 한다 (최초 호출때만 사용한다. 싱글톤)
		...
		// 5) java.sql.Dte는 new java.sql.Date(new java.util.Date().getTime())로 받는다.
...
//		java.sql.Date d2 = new java.sql.Date(time);
	...
	// saveDirectory 내부의 파일이름을 문자열로 배열형태로 반환하는 함수
